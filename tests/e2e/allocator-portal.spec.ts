...
const DEMO_EMAIL = "demo.allocator@alpinedd.com";
const DEMO_PASSWORD = "demo123";

// ── Public surface — needs only the dev server ───────────────────────────────
test.describe("Allocator portal — public surface", () => {
  test("/login renders the allocator login form with no public signup", async ({ page }) => {
...
// ── Authenticated flow — requires the allocator migrations + seed to be run ──
// (020_allocator_portal.sql + 021_allocator_seed.sql applied to Supabase).
test.describe("Allocator portal — authenticated flow (requires seeded DB)", () => {
  ...
  test("demo allocator logs in and sees the two seeded reports", async ({ page }) => {
    await login(page);
