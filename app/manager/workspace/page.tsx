...
import {
  getResponses, chapterCompletionPct, lastSavedRelative, type Response,
...
export default function WorkspaceOverviewPage() {
  ...
  useEffect(() => {
    fetch("/api/manager/me")
      .then((r) => {
        if (!r.ok) { router.replace("/manager/login"); return null; }
        return r.json();
      })
      .then((data) => {
        if (!data) return;
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {answered === 0
            ? `Welcome to Alpine, ${firmName.split(/[\s,]/)[0]}.`
            : `${overallPct}% complete.`}
        </h1>
        <p
          ...
          return (
            ...
            >
              ...
              <p className="font-mono text-[10px] mt-2" style={{ color: MUTED, letterSpacing: "0.04em" }}>
                {Object.keys(getResponsesForChapter(ch.num)).length}/{ch.questions.length} answered
              </p>
            </Link>
          );
        })}
...
function getResponsesForChapter(chapterNum: number): Record<string, Response> {
  if (typeof window === "undefined") return {};
  const all = getResponses();
  return Object.fromEntries(Object.entries(all).filter(([, r]) => r.chapterNum === chapterNum));
}
...
function RightPanel({ overallPct, answered, total, lastSaved }: {
  overallPct: number; answered: number; total: number; lastSaved: string | null;
}) {
  return (
