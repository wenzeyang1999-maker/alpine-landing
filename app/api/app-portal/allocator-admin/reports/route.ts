...
import { allReportEntries, getReportEntry, isValidReportSlug } from "@/lib/allocator/report-registry";
...
export async function GET(req: NextRequest) {
  ...
  if (error) {
    console.error("[allocator-admin/reports] list error:", error);
...
export async function POST(req: NextRequest) {
  ...
  if (publish) {
    ...
    if (error) {
      console.error("[allocator-admin/reports] publish error:", error);
      return NextResponse.json({ error: "Couldn't publish the report." }, { status: 500 });
    }
    await logAudit({ actor: admin, action: "allocator.report.publish", target: slug });
  } else {
    const { error } = await supabase.from("report_publications").delete().eq("report_slug", slug);
    if (error) {
      console.error("[allocator-admin/reports] unpublish error:", error);
      return NextResponse.json({ error: "Couldn't unpublish the report." }, { status: 500 });
    }
    await logAudit({ actor: admin, action: "allocator.report.unpublish", target: slug });
  }
