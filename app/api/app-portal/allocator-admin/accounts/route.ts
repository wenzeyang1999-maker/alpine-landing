...
export async function GET(req: NextRequest) {
  ...
  if (error) {
    console.error("[allocator-admin/accounts] list error:", error);
    return NextResponse.json({ error: "Couldn't load accounts." }, { status: 500 });
  }
  return NextResponse.json(data ?? []);
}

// POST — create an allocator account. { email, full_name, organization, password }
export async function POST(req: NextRequest) {
  ...
  if (error) {
    ...
    console.error("[allocator-admin/accounts] create error:", error);
    return NextResponse.json({ error: "Couldn't create the account." }, { status: 500 });
  }

  await logAudit({ actor: admin, action: "allocator.account.create", target: email });
  return NextResponse.json(data);
}
...
export async function PATCH(req: NextRequest) {
  ...
  if (error || !data) {
    console.error("[allocator-admin/accounts] patch error:", error);
    return NextResponse.json({ error: "Couldn't update the account." }, { status: 500 });
  }

  await logAudit({
    actor: admin,
    action: body.is_active ? "allocator.account.activate" : "allocator.account.deactivate",
    target: data.email,
  });
  return NextResponse.json(data);
}
