...
interface Upload {
  ...
  allocator_id: string;
  allocators: { email: string; full_name: string | null; organization: string | null } | null;
}

const RATING_COLOR: Record<string, string> = { GREEN, YELLOW: AMBER, RED: "#EF4444" };

const REPORTS_API = "/api/app-portal/allocator-admin/reports";
const ACCOUNTS_API = "/api/app-portal/allocator-admin/accounts";
const ASSIGNMENTS_API = "/api/app-portal/allocator-admin/assignments";
const DOCUMENTS_API = "/api/app-portal/allocator-admin/documents";
...
export default function AllocatorAdminPanel() {
  ...
  const loadCore = useCallback(async () => {
    ...
    try {
      ...
      setError("Couldn't load allocator portal data. Try refreshing.");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const isAssigned = (allocatorId: string, slug: string) =>
    assignments.some((a) => a.allocator_id === allocatorId && a.report_slug === slug);

  const toggleAssignment = async (allocatorId: string, slug: string, assign: boolean) => {
    setBusy(`asg-${allocatorId}-${slug}`);
    try {
      const res = assign
        ? await fetch(ASSIGNMENTS_API, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ slug, allocatorId }),
          })
        : await fetch(
            `${ASSIGNMENTS_API}?slug=${encodeURIComponent(slug)}&allocatorId=${encodeURIComponent(allocatorId)}`,
            { method: "DELETE" },
          );
      if (res.ok) {
        setAssignments((prev) =>
          assign
            ? [...prev, { allocator_id: allocatorId, report_slug: slug }]
            : prev.filter((a) => !(a.allocator_id === allocatorId && a.report_slug === slug)),
        );
      }
    } finally {
      setBusy(null);
        ...
        <div className="space-y-4">
          {publishedReports.length === 0 ? (
            ...
            <>
              ...
              {accounts.length === 0 ? (
                <p className="font-body text-sm" style={{ color: MUTED }}>
                  Create an allocator account before assigning reports.
                </p>
              ) : (
                <div className="space-y-2">
                  {accounts.map((a) => {
                      ...
                      <div className="flex items-center justify-between gap-4 flex-wrap">
                        <div className="flex items-start gap-3 min-w-0">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="font-body text-xs mt-0.5" style={{ color: MUTED }}>
                              {u.allocators?.email ?? "unknown allocator"}
                              {u.allocators?.organization ? ` · ${u.allocators.organization}` : ""}
                              {" · "}
                        ...
                        <div className="flex items-center gap-3 shrink-0">
                          ...
                          <a
                            href={`/api/app-portal/allocator-admin/document-file?id=${encodeURIComponent(u.id)}`}
                            target="_blank"
