...
import { TOPIC_DATA, RIDGELINE_MOCK, VAULT_DATA } from "@/lib/app-portal/ridgeline-data";
import { ReportViewer } from "@/components/app-portal/engine-stubs";
import { PlaceholderTab } from "./PlaceholderTab";
import { RefDot } from "./RefDot";
import { downloadDemoFile } from "@/lib/app-portal/demo-downloads";
...
export function TopicPage({ topicNumber, onNavigate, alpineReviewId, topicDataOverride, mockOverride, vaultDataOverride, slug, onRatingChange, initialRating, initialRiskOverrides, onRiskObsSaved }: { topicNumber: number; onNavigate: (page: string) => void; alpineReviewId?: string | null; topicDataOverride?: Record<number, any>; mockOverride?: any; vaultDataOverride?: any; slug?: string; onRatingChange?: (topicNumber: number, rating: string) => void; initialRating?: string; initialRiskOverrides?: Record<string, RiskObsOverride>; onRiskObsSaved?: (id: string, edit: RiskObsOverride) => void }) {
  ...
  async function saveReportEdit() {
    ...
    if (chapterDraftSlug) {
      try {
        const res = await fetch("/api/app-portal/report-draft", {
          method: "PUT",
  ...
  useEffect(() => {
    ...
    fetch(`/api/app-portal/report-draft?slug=${encodeURIComponent(chapterDraftSlug)}`)
      .then(r => {
  ...
  useEffect(() => {
    ...
    fetch(`/api/app-portal/reviews/${alpineReviewId}/output/content?report_type=full`, { credentials: "include" })
  ...
  async function saveRisk(id: string) {
    ...
    setSavingRisk(true);
    try {
      await fetch("/api/app-portal/risk-obs", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  async function saveRating() {
    ...
    try {
      await fetch("/api/app-portal/topic-rating", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
