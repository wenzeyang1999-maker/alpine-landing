...
import { FOLLOW_UP_MOCK, COLLECTION_DOCS } from "@/lib/app-portal/ridgeline-data";
import { TRELLIS_COLLECTION_DOCS } from "@/lib/app-portal/trellis-data";
import { AURORA_COLLECTION_DOCS } from "@/lib/app-portal/aurora-data";
...
async function pushToServer(slug: string, state: PendingState, userEmail: string) {
  if (!userEmail) return;
  try {
    await fetch("/api/app-portal/notes", {
      method: "POST",
...
export function FollowUpSection({ mock, slug }: { mock: typeof FOLLOW_UP_MOCK; slug?: string }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/app-portal/notes?slug=${slug}&user=${encodeURIComponent(userEmail)}`, { headers: { "x-notes-token": TOKEN } })
