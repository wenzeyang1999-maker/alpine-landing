...
export default function DocumentsPanel({ slug }: { slug: string }) {
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [docs, setDocs] = useState<AllocatorDoc[]>([]);
  ...
  const loadDocs = useCallback(async () => {
    setLoadError(false);
    try {
      const res = await fetch(`/api/allocator/documents?slug=${encodeURIComponent(slug)}`);
  ...
  const handleUpload = useCallback(
    async (files: FileList | File[]) => {
      ...
      for (const file of list) {
        ...
        try {
          const res = await fetch("/api/allocator/documents", { method: "POST", body: formData });
          if (!res.ok) {
            const data = await res.json().catch(() => null);
            setUploadError(data?.error || "Upload failed — try again.");
            continue;
          }
          const row: AllocatorDoc = await res.json();
  ...
  const handleDelete = useCallback(async (id: string) => {
    try {
      const res = await fetch(`/api/allocator/documents?id=${encodeURIComponent(id)}`, {
        method: "DELETE",
      });
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="font-body text-sm mt-1" style={{ color: MUTED }}>
          Upload supporting documents for this review. Your Alpine analyst is
          notified and will incorporate them — each document shows whether it
          has been processed into the report yet.
        </p>
      </div>
      ...
      {uploadError && (
        <p className="font-body text-sm" style={{ color: "#B91C1C" }} aria-live="polite">
          {uploadError}
        </p>
      )}
              ...
              >
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke={VIOLET} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="shrink-0" aria-hidden>
